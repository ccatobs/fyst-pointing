"""Custom exceptions for the fyst-pointing library.
...
class PointingWarning(UserWarning):
    """Base warning class for fyst-pointing.

    Allows users to filter fyst-pointing warnings specifically::
...
class PointingError(ValueError):
    """Base exception for all fyst-pointing errors.
    ...
    >>> from fyst_pointing.exceptions import PointingError
    >>> try:
...
class TrajectoryBoundsError(PointingError):
    ...
    >>> from fyst_pointing.exceptions import TrajectoryBoundsError
    >>> try:
    ...     trajectory.validate(site)
